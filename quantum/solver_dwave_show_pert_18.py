...
def main(args):
    ...
    for data in test_loader:
        if i in images_found:
            ...
            plt.matshow(img.reshape((28,28)), cmap='gray', norm=NoNorm())
            plt.savefig('img.pdf')
            plt.matshow(perturb.reshape((28,28)), cmap='gray', norm=NoNorm())
            plt.savefig('perturb.pdf')
            
            img_new = img + perturb
            plt.matshow(img_new.reshape((28,28)), cmap='gray', norm=NoNorm())
            plt.savefig('img_new.pdf')
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.info(
            "Successfully ran D-Wave's Hybrid CQM Solver for image 18.")
